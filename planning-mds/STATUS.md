...
W025 (C1) and W026 (C2) are complete; W026 is at the §6 checkpoint 2 pause. The validator, the four reconciled ADRs (`identifier-strategy.md`, `status-promotion.md`, `null-semantics.md`, `data-classification.md`), and the canonical hardening plan now agree on the same rule set. No contract was edited in C1 or C2; the punch list at `planning-mds/CANONICAL_HARDENING_PUNCH_LIST.md` lists 203 findings against the unmodified 0.2.0 surface, and that list is the input to C3.

Reviewer-driven amendments applied during the C2 checkpoint:

- **`source_system_code` binds to a real codeset.** `identifier-strategy.md` now treats it like every other `*_code` field. The `source-system-code` codeset is added to the C5.1 slate. C3 wires the relationship rather than carrying a per-contract exemption. This adds one row to C5.1 and does not enlarge C3 scope (the relationship-add is part of C5.2's scripted sweep — C3 just stops adding exemptions for it).
- **C1.8 widened to include `_uid` suffix.** Opaque GUIDs are practically never PII per `identifier-strategy.md`, so a `_uid` tagged `RESTRICTED + PII` is almost certainly a mis-tag. The widening caught `lifecycle-event-type.resulting_lifecycle_status_uid` automatically — exactly the field plan §4 C3.5 named. Validator now flags 5 fields (was 4) at `RESTRICTED + PII` for reviewer attention; the C3.5 fix list is unchanged.

Items the reviewer should confirm before C3 begins:

- **ADR direction.** The C2 default ("rewrite ADR to match shipped contracts when the shipped contract is the better answer") was applied. Confirmed.
- **C1.1 width** — strict same-prefix matcher catches 6 amount/currency pairings, not the 4 listed in `CANONICAL_HARDENING_PLAN.md` §4 C3.4. The two extras (`annualized_premium_amount`, `premium_change_amount`) are real same-shape issues; recommend folding into C3.4 when C3 runs.

Once C3 is given the green light, W027 starts: `scripts/refactor/apply-hardening-c3.py` plus the punch-list-driven contract edits per `§4 phase C3`. C3 is the first phase that touches contract YAML; expected output is a single bulk commit bumping every touched contract to 0.3.0 with named changelog entries.

Sequencing remains: **W025 (C1, done) → W026 (C2, done; paused) → W027 (C3) → W028 (C4) → W029 (C5) → W030 (C6) → W031 (C7) → W023 (Fabric F1–F8) → Milestone 10**. Pause-for-review checkpoints after C1, C2 (active now), and C4. W023 (Fabric) does not begin until W031 closes.
