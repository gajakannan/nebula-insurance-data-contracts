...
| W025 | Canonical hardening C1 — validator-first enforcement | Not started | Add 12 new validator rules covering currency pairing, codeset relationship resolution, target-contract-id resolution, append-only field bans, classification heuristics, status-promotion gates, changelog-on-version-bump, ADR-id resolution. No contract edits in this phase; emits a deterministic violation report against the 0.2.0 surface. Detail: `planning-mds/CANONICAL_HARDENING_PLAN.md` §4 phase C1. Pause for review after completion. |
...
W015–W020 are complete; the 0.2.0 canonical surface passes the current validator. W021 (risk-transfer family) is deliberately deferred. Internal validation surfaced a canonical-layer backlog that is captured as W025–W031 (canonical hardening, Milestone 8.5).

The active focus is **W025 — canonical hardening C1 (validator-first enforcement)** per `planning-mds/CANONICAL_HARDENING_PLAN.md`. C1 adds 12 new validator rules without editing any contract, producing a deterministic violation report that drives the C3 bulk refactor.

Sequencing: **W025 (C1) → W026 (C2) → W027 (C3) → W028 (C4) → W029 (C5) → W030 (C6) → W031 (C7) → W023 (Fabric F1–F8) → Milestone 10**. Pause-for-review checkpoints are baked in after C1 (validator-rule semantics), C2 (ADR text changes), and C4 (entity-gap decisions on `Account`/`Agreement` and `PolicyFinancialTransaction`). W023 (Fabric) does not begin until W031 closes.
